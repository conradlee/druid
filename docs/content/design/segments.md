...
interval, where the duration of a time inteval is configured using the
`segmentGranularity` parameter of the `granularitySpec`.  For Druid to
...
changing the the granularity of the time interval or further sharding
your data and tweaking the `targetPartitionSize` in your
`partitioningSpec` (a good starting point for this parameter is 5
million rows).  See the 'Partitioning specification' section of the
[Batch ingestion](../ingestion/batch-ingestion.html) documentation for
more information on segment granularity and sharding.
